...
def main():
    ...
    if args.task_mode == 'data2text':
        ...
        if QUICK_CHECK:
            ...
            if ('lowdata' in args.model_name_or_path) or (args.prefixModel_name_or_path is not None and 'lowdata' in args.prefixModel_name_or_path):
                test_path = '../e2e_data/src1_valid.txt'
            else:

                if args.eval_dataset == 'valid':
                    test_path = '../e2e_data/src1_valid.txt'
                elif args.eval_dataset == 'test':
                    test_path = '../e2e_data/src1_test.txt'
        ...
        if args.task_mode == 'webnlg':
            ...
            # test_path = "../DART/dart/data/v1.1.1/dart-v1.1.1-full-dev.json"
        ...
        if QUICK_CHECK:
            ...
            decode_mode = 'beam'
            curr_dir = os.path.join('webnlg_models/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, decode_mode))
            print(curr_dir)
            gold_dir = os.path.join('webnlg_models/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, 'gold'))
            print(gold_dir)
            write_e2e_corr(prompt_text_pair, prompt_text_dict, gold_dir)
            src_dir = os.path.join('webnlg_models/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, 'src'))
        ...
        if args.task_mode == 'classify-sentiment':
            ...
            test_path = "../contrast_LM/transformers/examples/text-classification/glue_data/AG-news/dev1.tsv"
        ...
        if QUICK_CHECK:
            ...
            decode_mode = 'greedy'
            curr_dir = os.path.join('classify_models/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, decode_mode))
            # curr_dir = '../contrast_LM/transformers/examples/text-generation/classify_results/{}_{}_{}'.format(
            #     temp, split_file, decode_mode)
            print(curr_dir)
            gold_dir = os.path.join('classify_models/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, 'gold'))
            # gold_dir = '../contrast_LM/transformers/examples/text-generation/classify_results/{}_{}_{}'.format(
            #     temp,
            #     split_file,
            #     'gold')
            print(gold_dir)
            write_e2e_src(prompt_text_tgt, gold_dir)
            src_dir = os.path.join('classify_models/',
                                   args.gen_dir,
                                   '{}_{}_{}'.format(temp, split_file, 'src'))
            # src_dir = '../contrast_LM/transformers/examples/text-generation/classify_results/{}_{}_{}'.format(
            #     temp,
            #     split_file,
            #     'src')
            write_e2e_src(prompt_text_lst, src_dir)
        ...
        if args.task_mode == 'cnndm':
            test_path = "../contrast_LM/transformers/examples/seq2seq/cnn_dm/test.source"
            max_source_length = 512
            max_target_length = 142
            prompt_text_dict = read_sum_files(test_path, tokenizer, max_source_length, max_target_length)
        elif args.task_mode == 'xsum':
            test_path = "../contrast_LM/transformers/examples/seq2seq/xsum/test.source"
            max_source_length = 512
        ...
        if QUICK_CHECK:
            ...
            split_file = 'test' # test
            decode_mode = 'beam'
            curr_dir = os.path.join('summarization_models/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, decode_mode))


            print(curr_dir)
            gold_dir = os.path.join('summarization_models/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, 'gold'))
            print(gold_dir)
            write_e2e_corr(prompt_text_lst, prompt_text_dict, gold_dir)
    ...
    if args.task_mode == 'data2text':
        ...
        os.system("bash ../DART/dart/evaluation/run_eval_on_webnlg.sh "
        ...
        print(curr_dir)
        print(gold_dir)
        temp_command = "python ../classify-eval/eval.py {} {}".format(gold_dir, curr_dir)
        print(temp_command)
    ...
    if args.task_mode == 'topic' or args.task_mode == 'sentiment':
        ...
        temp_command = "python ../attribute-eval/eval.py {} ".format(curr_dir)
        print(temp_command)
        os.system(temp_command)
